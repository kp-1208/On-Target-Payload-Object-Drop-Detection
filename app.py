...
first_cam = list(IP_CAMS.keys())[0]
vid_path = IP_CAMS[first_cam][0] #path to first cam's IP
...
def get_camera_by_ip(ip_address):
    for cam_name, cam_info in IP_CAMS.items():
        if cam_info[0] == ip_address:
            return cam_name, cam_info
    return None, None
...
def generate_raw_frames():
    ...
    while True:            
        ...
        if success:
            ...
            if validators.url(vid_path):     
                # create masking for region of interest
                cam_name, cam_info = get_camera_by_ip(vid_path)
                roi_left, roi_top, roi_right, roi_bottom = roi_coordinates(cam_info[1][0],
                        cam_info[1][1], frame.shape[1], frame.shape[0], cam_info[1][2], 
                        cam_info[1][3])
                mask = create_mask(frame_copy, roi_left, roi_top,
